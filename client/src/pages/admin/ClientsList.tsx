...
import { usersData, phasesData } from "@/lib/mock-data";
import { Card, CardContent } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Search, UserPlus, ChevronRight, Activity, AlertCircle } from "lucide-react";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";

export default function AdminClientsList() {
  const [search, setSearch] = useState("");
  const clients = usersData.filter(u => u.role === 'Client' && u.name.toLowerCase().includes(search.toLowerCase()));

  const getClientStatus = (clientId: string) => {
    const clientPhases = phasesData.filter(p => p.clientId === clientId);
    const activePhase = clientPhases.find(p => p.status === 'Active');
    const pendingPhase = clientPhases.find(p => p.status === 'Waiting for Movement Check');
  ...
  return (
    <div className="space-y-8 animate-in fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-slate-900 tracking-tight">Client Management</h1>
        ...
        {clients.map(client => {
          ...
          return (
            <Link key={client.id} href={`/app/admin/clients/${client.id}`}>
              <Card className="border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-all cursor-pointer group bg-white overflow-hidden rounded-2xl">
