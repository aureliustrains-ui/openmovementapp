    ...
    return (
      <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4">
        <div className="text-center mb-10 mt-8">
          ...
          <h1 className="text-3xl font-display font-bold text-slate-900 tracking-tight">Movement Check Required</h1>
        ...
        <div className="space-y-4">
          {pendingPhase.movementChecks.map((mc, i) => (
            <Card key={i} className="border-2 border-slate-200 shadow-sm rounded-2xl overflow-hidden bg-white">
  ...
  if (!activePhase) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        ...
        <h2 className="text-2xl font-display font-bold text-slate-900">No Active Phase</h2>
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in">
      <div className="bg-slate-900 rounded-3xl p-8 md:p-10 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <Badge className="bg-white/10 text-white border-white/20 hover:bg-white/20 backdrop-blur-md mb-6 py-1.5 px-3">
            Active Phase • Week 1 / {activePhase.durationWeeks}
          </Badge>
          <h1 className="text-4xl md:text-5xl font-display font-bold tracking-tight mb-4">{activePhase.name}</h1>
          ...
          {activePhase.schedule.filter(s => s.week === 1).map((sched, i) => {
            ...
            return (
              <Link key={i} href={`/app/client/session/${session?.id}`}>
                <Card className="border-slate-200 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all cursor-pointer group bg-white rounded-2xl overflow-hidden h-full">
                    ...
                    <div className="flex items-stretch h-full">
                      ...
                      <div className="p-6 flex-1 flex justify-between items-center">
                        <div>
                          ...
                          <p className="text-sm text-slate-500 mt-1">{session?.sections.length} Blocks • {session?.description}</p>
                        </div>
